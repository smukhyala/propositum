...
scenarios, two of which seal `shouldRaise: false`, so the false-stop arm is exercised.~~
**Corrected 2026-09-03 — `src/eval/index.ts` is what knows how many.** Scenarios sealing
`shouldRaise: false` exercise the false-stop arm, and `evening-classes` seals a structural rule, so
both directions of `wrong-rule` are reachable. What still holds:
