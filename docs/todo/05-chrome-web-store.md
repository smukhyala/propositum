...
**Status:** ~~not started~~ **item 1 of the work done 2026-09-03 — the key is
pinned; the rest not started, and one job the pinning left behind is added
under *What you have to do yourself*.**
...
| **The private key** *(added 2026-09-03)* | **Move `propositum-extension.pem` somewhere you keep.** The keypair behind the manifest's `key` was generated by an agent on 2026-09-03 and the private half was written to that session's scratchpad — `/private/tmp/claude-501/-Users-markshi-berkeley-propositum/b144ce90-746e-4391-a705-b66be052e854/scratchpad/propositum-extension.pem` on the machine it ran on — which is a temporary directory, not a place anything is kept. Nothing in the product reads it and only packing a `.crx` needs it, so losing it costs nothing until that day; on that day it costs a new key, a new id, and every install. `*.pem` is ignored by git since the same date, so the repository is not the place either. | free | minutes, and only a person can do it |
...
1. ~~Pin the `key` — item 7 of [`04`](./04-quick-fixes.md) — and confirm the id it
   produces matches whatever is already in anybody's `.env`.~~ **Pinned
   2026-09-03. It does not match, and could not have: every `.env` written
   before that day holds the old, path-derived id, and the paired row on the
   machine it was pinned from names the same one. Re-pair once on `/first-run`
   and let it rewrite the row — `extension/README.md`, *Before a real install*.**
