...
| ~~**Nothing yet**~~ `tests/schema-migrations.test.ts` | ~~**That a schema change ships with a migration.** A `prisma migrate diff` against the schema would catch it, and it is not written.~~ **Written the same day.** `migrate diff --from-migrations … --exit-code` replays the migrations into a shadow database and compares. It is a semantic comparison rather than a check that a file was added, because *"a migration exists"* passes for a migration that does not say what the schema says, which is the actual defect. A second case manufactures drift and asserts the guard notices — a comparison that always reports "no difference" is a green tick that has stopped meaning anything. |
