...
| `vault`, `wallet`, `password`, `passphrase`, `credential` *as a field, type or column — added 2026-08-26 ([ADR-0025](docs/adr/0025-computer-use-beyond-the-browser.md))* | **nothing.** There is no field for one and that is the design: Chrome holds the person's passwords and Propositum clicks the prompt, so it never sees, stores or transmits a credential of theirs. The words are banned because a name is how a field arrives — `fill-credential` was the shape an earlier draft proposed, and refusing the word refuses the shape. Correct in **prose**, in this table, and in ADR-0025 §5 where the vault is argued for and declined |
