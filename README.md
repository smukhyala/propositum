...
| [`docs/adr/`](./docs/adr/) | ~~Seven decisions~~ ~~eleven, corrected 2026-08-16~~ ~~15 decisions, corrected 2026-08-19~~ ~~18 decisions — three landed 2026-08-20 with the everyday-computing direction~~ ~~19 decisions — [ADR-0019](./docs/adr/0019-disclosure-and-what-may-never-fold.md) landed 2026-08-22 with the interface simplification and is the newest~~ ~~20 decisions — [ADR-0020](./docs/adr/0020-remembering-a-decline.md) landed 2026-08-22 with offer reticence and is the newest~~ ~~23 decisions — three landed together 2026-08-26 with the phone thread and the menu-bar app: [ADR-0021](./docs/adr/0021-a-thread-on-the-persons-phone.md), [ADR-0022](./docs/adr/0022-the-fourth-verdict.md) and [ADR-0023](./docs/adr/0023-the-tray-app-owns-the-runtime.md), the last of which is the newest~~ ~~26 decisions — three more landed the same day, and together they are the largest reversal in the series: [ADR-0024](./docs/adr/0024-purchases-within-a-ratified-authorisation.md) lets Propositum buy things, [ADR-0025](./docs/adr/0025-computer-use-beyond-the-browser.md) takes it out of the browser and onto the machine, and [ADR-0026](./docs/adr/0026-reading-a-one-time-code.md) lets it read a 2FA code out of Messages. ADR-0023 was amended by them (struck earlier: "two days after") the same day it was accepted~~ ~~27 decisions — [ADR-0027](./docs/adr/0027-a-sealed-bundle-and-where-the-state-moves.md) landed 2026-08-28 with the signed `.dmg`, seals the bundle, moves an installed copy's state to Application Support, and refuses the update feed for now; it is the newest~~ ~~28 decisions — [ADR-0028](./docs/adr/0028-a-capped-key-ships-in-the-bundle.md) landed 2026-08-29 with the todo 09 design: a tester build may carry a spend-capped bundled key, so the first run stops asking for one; it is the newest~~ ~~27 decisions on this branch — corrected 2026-09-01: [ADR-0029](./docs/adr/0029-the-mailbox-and-a-calendar-of-our-own.md) reopens ADR-0014 for the mailbox and a calendar of our own, and its number deliberately skips 0027 and 0028, which are accepted on the tray-release branch and arrive with its merge. Before it, three landed together on 2026-08-26, the largest reversal in the series: [ADR-0024](./docs/adr/0024-purchases-within-a-ratified-authorisation.md) lets Propositum buy things, [ADR-0025](./docs/adr/0025-computer-use-beyond-the-browser.md) takes it out of the browser and onto the machine, and [ADR-0026](./docs/adr/0026-reading-a-one-time-code.md) lets it read a 2FA code out of Messages. ADR-0023 was amended by them (struck earlier: "two days after") the same day it was accepted~~ ~~29 decisions — the two branches met 2026-09-02: [ADR-0027](./docs/adr/0027-a-sealed-bundle-and-where-the-state-moves.md) and [ADR-0028](./docs/adr/0028-a-capped-key-ships-in-the-bundle.md) arrived with the tray-release merge, beside [ADR-0029](./docs/adr/0029-the-mailbox-and-a-calendar-of-our-own.md), which reopens ADR-0014 for the mailbox and a calendar of our own and is the newest by number.~~ ~~30 decisions — corrected 2026-09-02: [ADR-0030](./docs/adr/0030-a-halt-closes-the-question.md) decides what stopping a run parked on a question does, which until then was nothing at all, and is the newest.~~ ~~31 decisions — corrected 2026-09-02, later the same day: [ADR-0031](./docs/adr/0031-a-first-look-is-progress.md) makes a first read of something progress, so `no-progress` stops halting a plan that reads several things before it writes — the failure two paid eval runs produced. It is the newest.~~ ~~32 decisions — corrected 2026-09-03: [ADR-0033](./docs/adr/0033-a-late-tick-is-a-slept-machine.md) makes `machine_slept` writable off the lateness of the gap sweeper's own tick, and refuses the tray wake notification that would have been the obvious way to do it. It is the newest.~~ ~~33 decisions — corrected 2026-09-03, later the same day, and the 32 above was right on the branch that wrote it: [ADR-0032](./docs/adr/0032-a-page-from-a-source-already-approved.md) lets a document come in from a host, and only from an origin the project already approved, and [ADR-0033](./docs/adr/0033-a-late-tick-is-a-slept-machine.md) makes `machine_slept` writable off the lateness of the gap sweeper's own tick, refusing the tray wake notification that would have been the obvious way to do it. Each was written without sight of the other and each counted without it — the same collision the `SendAuthorization` row two cells above records, and the reason the count lives here where a test can read it.~~ ~~37 decisions — corrected 2026-09-07: four landed together with the between-sittings direction, and none of them builds a sensor. [ADR-0034](./docs/adr/0034-somewhere-to-put-an-event-outside-a-sitting.md) gives an event that did not happen inside a sitting somewhere to go, which is the sentence three other documents said could not be written; [ADR-0035](./docs/adr/0035-what-a-person-said-they-are-waiting-on.md) lets a person say what they are waiting on and lifts the strike on `waiting` as a lifecycle word; [ADR-0036](./docs/adr/0036-ordering-candidates-without-a-score.md) orders candidates across kinds without inventing a score, paying a debt ADR-0018 recorded; and [ADR-0037](./docs/adr/0037-was-the-offer-any-good.md) asks whether a reading that can see between sittings beats one built from a single sitting — the question the 2026-08-27 baseline result left open. ADR-0037 was the newest.~~ ~~38 decisions — corrected 2026-09-08:~~ **39 decisions — corrected 2026-09-08, later the same day: [ADR-0039](./docs/adr/0039-a-migration-history-and-a-copy-before-it.md) gives the database a migration history and takes a copy of the file before touching it, replacing the `prisma db push` the tray ran at every launch — a development command that reconciles the file to the schema, asks nobody, and on a narrowing change drops the column and the data in it. It is the newest. Before it, [ADR-0038](./docs/adr/0038-deleting-a-project.md) decides that a person may delete a project and that it takes its ledgers with it, which means ~~thirteen~~ **fourteen** append-only tables lose their no-`DELETE` guard and keep the other two. The argument is `append-only.ts`'s own, about `ActionEvidence`, generalised: *immutability is about rewriting history, retention is about how long history is kept, and only the second needs `DELETE`*. ~~Decided, not built.~~ **Built the same day** — `deleteWithEverything`, `deleteProject`, *Delete this project* on the project screen, and `tests/delete-project.test.ts`, which counts every table in the database afterwards rather than the ones somebody remembered. Two things the build corrected in the ADR: the count was thirteen and is fourteen (`external_event` arrived from ADR-0034 the same day and both counts were taken without it), and a mis-ordered cascade does **not** abort on a foreign key — Prisma nullifies the child instead, silently, which is why the order is documented rather than tidy. It is the newest.****, each with the option it rejected and why. The number went stale four ADRs ago, was corrected, and went stale again by four within three days — because nothing counted them. Something counts them now, which is why this correction was made in the same commit as the ADR rather than three days after it. |
...
not**, because elapsed silence cannot tell a slept machine from a dead service worker~~
**corrected 2026-09-03 ([ADR-0033](./docs/adr/0033-a-late-tick-is-a-slept-machine.md)):
`machine_slept` is writable too.** Elapsed silence still cannot tell the two apart, and the
lateness of the sweeper's own tick can — a dead service worker does not stop the app process
being scheduled, and a suspended machine stops everything. All four reasons can now occur; and a
...
npm run db:prepare            # copies the file, then migrates it. Creates one if there is none
npm run dev                   # serves on 3117, and starts the worker beside it
```

~~`npx prisma db push`~~ **`npm run db:prepare`, since 2026-09-08 —
[ADR-0039](./docs/adr/0039-a-migration-history-and-a-copy-before-it.md).** `db push` reconciles the
file to the schema and asks nobody, which is right for a throwaway database and wrong for the only
copy of somebody's work: a narrowing change drops the column and the data in it. `db:prepare` copies
the file first, brings a database that predates migrations onto the history without re-running it,
and then applies what is pending. `npm run db:push` still exists and the test suite still uses it,
because every database the suite builds is thrown away.
...
Whenever the schema changes, ~~`prisma db push`~~ **a migration** rebuilds the affected table and
**silently drops its append-only triggers**. They are reinstalled and verified at the next app
startup; restart before trusting the database. *(2026-09-08: the command changed and this hazard did
not — `migrate deploy` drops them on a table rebuild exactly as `db push` did, which is why the
reinstall-and-verify at startup is the mechanism rather than a belt-and-braces.)*
